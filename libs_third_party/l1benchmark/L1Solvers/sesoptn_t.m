function [x,diff_x, tt, ids, report] =sesoptn_t(x, x00, t00)
...
if isempty(par.multA),    par.multA     = @(x,par) x;end
if isempty(par.multAt), par.multAt  = @(x,par) x;end
if isempty(par.func_u),  par.func_u  = @dummy_func_u;end
...
%          
      ...
      %figure;plot(P);
      if ~options.FlagWolfeLineSearch, R=par.multA(P,par); end
	...
	elseif options.FlagFISTA 
      ...
      %
		...
		if sesop_iter>1,  % Put previous normalized step as a new direction
			...
			if mod(sesop_iter,options.PeriodRestoreAp)==0 %|| mod(sesop_iter+1,options.PeriodRestoreAp)==0,
				r1=par.multA(p,par);        % This line added  to prevent error accumulation
				%fprintf(' norm(r1-r)=%g \n',norm(r1-r));
				r=r1;
			end
		...
		Ad_new_norm= par.multA(d_new_norm,par);
		...
		if options.FlagNemirovski_dir,
			if sesop_iter==1,
				...
				d1Nem=x-x0;
				Ad1Nem=par.multA(d1Nem,par);   % Durty way: this multiplication can be saved
			...
			Ad2Nem=par.multA(d2Nem,par);   % Durty way: this multiplication can be saved
			ind_new=ind_new+1;
      ...
      %
		...
		if options.max_iter_CGinTN > 0,
			...
			if sesop_iter>1,
				Ap=r;
				[f_u,g_u,h1Ap]=par.func_u(u,Ap,par);  % h1Ap - hess in u multiply Ap
				par.flagXnew=0;  % Only u was changed last time
				[f2,g2_x,h2p]=par.func_x(x,p,par);
				Hp=par.multAt(h1Ap,par)+h2p;
			end
			...
			if options.FlagRestrictedMatrix_TN
				...
				multA_tn  = par.multA;
				multAt_tn  = par.multAt;
				func_u_tn = par.func_u;
			end
			...
			if ~options.PureTN      % Two directions below are not used by "Classical"  TN
				...
				R(:,ind_new)=tmp*par.multA(d_new_tn,par);
   ...
   u_old=u;
	...
	if options.max_newton_iter>0,
		...
		if mod(sesop_iter+1,options.PeriodRestoreAx)==0,  %sesop_iter>1 % && ~options.FlagNonlinCG
			...
			%fprintf(' norm(u1-u)=%g \n',norm(u1-u));
			%u=u1; %fprintf('***');
			u=par.multA(x,par);   % This line added  to prevent error accumulation
		end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	function testgrad
		...
		x=rand(size(x)); Ax=par.multA(x,par);
		y=rand(size(Ax)); Aadj_y=par.multAt(y,par);
		tmp1=Ax(:)'*y(:); tmp2=x(:)'*Aadj_y(:);
		fprintf('<Ax,y>=%g <x,Aadj_y>=%g ratio=%g \n', tmp1,tmp2, tmp1/tmp2);

		% Test grad & Hess
		u=rand(m,1);
		[f,g_u,H_u]=par.func_u(u,eye(m),par);
		[g_u_num, H_u_num]=ghnum(u,par.func_u,[],par);
		...
		x=rand(n,1);
		[f,g_x,H_x]=par.func_x(x,eye(n),par);
		[g_x_num, H_x_num]=ghnum(x,par.func_x,[],par);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	function [x,u]=NewtonOptimizationOverSubspace(x0nwt,Ax0nwt,P,R)
		...
		for i_newton=1:options.max_newton_iter
         ...
         [f_u,g_u,hR]=par.func_u(u,R,par);  % hR - hess in u multiply R

			if options.AnalyticHessXmult
				[f2,g2_x,hP]=par.func_x(x,P,par);
...
function [x,Ax,Hx, p,Ap,Hp,d_new,report] = ...
	CGforSesopTN(func_u, func_x, multA, multAt, b, d_new, Ad_new, p, Ap, Hp, x_nonlin, Ax_nonlin, options,par,varargin)
%
...
for Niter=1:options.max_iter_CGinTN,
	...
	Hd_new=multAt(h1Ad_new,par)+h2d_new;
