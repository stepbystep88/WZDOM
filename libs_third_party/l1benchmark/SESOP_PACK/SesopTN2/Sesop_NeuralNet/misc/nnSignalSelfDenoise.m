...
INIT_SIGNALS=1;          % 1 - generate new signals; 0 - use signals from the previous run  
INIT_DATA_MATRIX=0;      % 1 - generate new data matrix ;  0 - use data matrix from the previous run  
...
%%%%% Optimize with: 
FlagSesop =   0;   %  1 - sesoptn tool;   
FlagFminunc = 1;   %  1 - fminunc BFGS Quasi-Newton (0ptimization toolbox of MATAB)
...
par.quadrpenpar=1e-2;       % Quadratic penalty for NN weights: used for regularization, to avoid overfitting
...
T=12000;           % Signal length:   the same for training and test signal
...
if INIT_DATA_MATRIX,
	...
	L= 2*delta;           % Interval around the reconstructed sample to be feed  into NN;
	...
	for t=1+delta: T-delta,   
		...
		Xtrain(:,k)=[SnoisyTrain(t-delta : t-1); SnoisyTrain(t+1:t+delta)];
		ytrain(k)=SnoisyTrain(t);
		%ytrain(k)=SnoisyTrain(t);
	end
	...
	for t=1+delta: T-delta,   
		...
		%ytest(k)=NoiseTest(t);
		Xtest(:,k)=[SnoisyTest(t-delta : t-1); SnoisyTest(t+1:t+delta)];
		ytest(k)=SnoisyTest(t);
	end
...
if FlagFminunc  %%%%%       Optimize using matlab optimization toolbox function fminunc:   BFGS quasi-Newton
	...
	options=optimset('LargeScale','off','GradObj','on','Display','iter','MaxIter',1000);
	tic
...
ind=1+delta:T-delta;
SdenoisedTrain = nnet(vcwb,par,Xtrain)';  
SdenoisedTest  = nnet(vcwb,par,Xtest)';
