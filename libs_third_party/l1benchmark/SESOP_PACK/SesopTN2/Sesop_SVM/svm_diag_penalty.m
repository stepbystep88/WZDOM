function  [fsum,g,HZ,diagH]=svm_diag_penalty(x,Z,par);
%SVM penalty for separation vector w (denoted x here):  mu*sum(|x(1:n-1)|), 
%    |.| is smoothed with parameter par.eps_smooth_abs:
%                       h(u) = abs(u) + 1./(abs(u)+1) - 1;
%                       f(x) = eps*h(x/eps)
%
% Call: [fsum,g,HZ,diagH]=svm_diag_penalty(x,Z,par)
%
% Input:
...
if nargout>2,  	   % Hessian-matrix product
	if  flagH
		mud2f = mu*(2/eps).*tmp1.*tmp; mud2f(n)=0;
		flagH=0;
	end
