function [x,status,history] = l1_ls_mz(A,varargin)
%
...
%       totalpcgmaxi : scalar; number of maximum PCG iterations (default:6000) M.Zibulevsky
%
% OUTPUT
...
defaults   = {1e-3,false,1e-3,5000,6000,zeros(n,1),ones(n,1),t0};
given_args = ~cellfun('isempty',varargin);
defaults(given_args) = varargin(given_args);
[reltol,quiet,eta,pcgmaxi,totalpcgmaxi,x,u,t] = deal(defaults{:});
...
for ntiter = 0:MAX_NT_ITER
    ...
    % M Zibulevsky: plot results
    parmz=Apar(A); parmz.current_iter=sum(pitrs); [fmz,gmz]=parmz.eval_fg(x,parmz);
    ...
    if sum(pitrs)>totalpcgmaxi, break; end %exit by total num of PCG iterations, Zibulevsky
...
elseif (ntiter == MAX_NT_ITER)
    ...
    status = 'Failed';
elseif (sum(pitrs)>totalpcgmaxi)
    % fail to find the solution within MAX_NT_ITER
    if (~quiet) disp('totalpcgmaxi exceeded.'); end
    status = 'Failed';
end
history = [pobjs-dobjs; pobjs; dobjs; sts; pitrs; pflgs];
