...
%          
	...
	elseif options.FlagFISTA 
      ...
      if options.FlagCoordDesc
         d_tmp=d_new;
         d_new=zeros(n,1);
         %[val,ind]=max(d_tmp.*2);
         ind=mod(sesop_iter-1,n)+1;
         %ind=randperm(n);ind=ind(1);

         d_new(ind)=sign(d_tmp(ind));
         Ad_new= multA(d_new,par);
         ind_new=ind_new+1;
			P(:,ind_new)=d_new;  % at the last free column
         R(:,ind_new)=Ad_new;
         
         
         for i=1:2  %n/8-1,
            d_new(ind)=0; ind=mod(ind,n)+1;
            d_new(ind)=sign(d_tmp(ind));
            Ad_new= multA(d_new,par);
            ind_new=ind_new+1;
            P(:,ind_new)=d_new;  % at the last free column
