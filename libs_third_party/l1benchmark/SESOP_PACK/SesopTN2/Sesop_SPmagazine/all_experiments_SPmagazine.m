...
%  flagSSF_LBFGS=1;
   ...
   %options.LBFGSmemory=8;       % Number of previous steps used in sesop LBFGS

   %flagSESOP_TN_10=1;
   %flagPCD_SESOP_TN_10=1;
   %flagPCD_SESOP_TN_10_PCD_inside_TN=1;
...
%%%%report_file_name=['reports_' report_name '_smooth_1e-3.mat'], save(report_file_name, 'reports', 'report_name');
%%%%report_file_name=['reports_' report_name '_smooth_1e-3_weight_1e-6.mat'], save(report_file_name, 'reports', 'report_name');
%%%%report_file_name=['reports_' report_name '_smooth_1e-8.mat'], save(report_file_name, 'reports', 'report_name');
%%%%report_file_name=['reports_' report_name '_concave.mat'], save(report_file_name, 'reports', 'report_name');
...
%c0=c;
n0=128;  m0=n0                   % Image size: m0-by-n0 

%X00=10*(sprandn(n0,n0,image_sparsity));save X00_sparse X00; % Uncomment and evaluate this once for given n0
...
%par.weight_abs_penalty                 = 0.7e-3; % Weight of smooth abs penalty (mu) -deblur?
%par.weight_abs_penalty                 = 4e-3; % Weight of smooth abs penalty (mu)
%if FlagDenoise, par.weight_abs_penalty = 0.4e-1; end 
if FlagDenoise, par.weight_abs_penalty = 0.4e-1; end 

%options.max_sesop_iter  = 40;    % Max  SESOP iterations
...
%i_run=0;clear reports; %reports.ProblName=par.ProblName;


%load reports_ComprSens_128.mat;  
%load reports_tomogr_128.mat;      
%load reports_deblur_128a.mat;  
...
% if flagPCD_SESOP_TN_10_PCD_inside_TN
% i_run=i_run+1;method='PCD-SESOP-TN-10-PCD-inside-TN', if FlagConcaveNorm, method=[method '-concave'],end 
% FlagSESOP=1; par.FlagPCD=1;par.FlagSSF=0;FlagMinFunc = 0;
% options.max_iter_CGinTN = 10; 
% period_show_progress_old=options.period_show_progress;
% %options.period_show_progress=ceil(options.period_show_progress/options.max_iter_CGinTN);
...
%options.period_show_progress=20;  % Periodicity of showing sesop and user plots
%par.weight_abs_penalty= 0.3e-3;

% 
% i_run=i_run+1;method='CG-concave',
% options.precond = 0; 
% FlagSESOP   = 1; par.FlagPCD=0;FlagMinFunc = 0;
% options.FlagNonlinCG = 1;    % 1 - Polak-Ribiere Nonlinear Conjugate Gradient method; 0 - any other method
% options.nLastSteps=1; 
% OptimizeBP; % Perform Optimization
% options.FlagNonlinCG = 0;    % 1 - Polak-Ribiere Nonlinear Conjugate Gradient method; 0 - any other method
...
% 
% i_run=i_run+1;method='SSF-CG-concave',
% options.precond = 1; FlagSESOP   = 1; par.FlagPCD=0;par.FlagSSF = 1;
% options.FlagNonlinCG = 1;
% options.nLastSteps=1; 
% OptimizeBP; % Perform Optimization
...
% 
% 
% i_run=i_run+1;method='PCD-CG-concave',
% FlagSESOP   = 1; par.FlagPCD=1;par.FlagSSF = 0;
% options.FlagNonlinCG = 1;    % 1 - Polak-Ribiere Nonlinear Conjugate Gradient method; 0 - any other method
% options.nLastSteps=1; 
% OptimizeBP; % Perform Optimization
...
% 
% i_run=i_run+1;method='PCD-concave',
% FlagSESOP   = 1; par.FlagPCD=1;FlagSSF = 0;options.max_newton_iter = 1;
% options.nLastSteps=0; 
% OptimizeBP; % Perform Optimization
% Report.method=method;  reports(i_run)=Report;
% options.max_newton_iter = 7;



% i_run=i_run+1;method='SESOP',
% options.precond = 0;         % 1 - use user-defined  preconditioner, 0 - don't
% FlagSESOP=1; par.FlagPCD=0;FlagMinFunc = 0;
% options.nLastSteps=1; 
% OptimizeBP;  % Perform Optimization
...
f_figure_handle=figure('Position',[00 40 500 800],'name',['Objective function' par.ProblName] );
