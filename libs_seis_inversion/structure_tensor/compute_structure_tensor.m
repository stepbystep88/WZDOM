...
%   U. K?the, "Edge and Junction Detection with an Improved Structure Tensor", 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% pre smoothing
m = min(options.window_size, round(min(n1, n2)/2)*2+1);
h = compute_gaussian_filter([m m],sigma1/(lambda*min(n1, n2)),[n1 n2]);
M = perform_convolution(M,h);
% M = medfilt2(M, options.medfilt2_size);
% compute gradient
...
if sub>1
    ...
    [XI,YI] = meshgrid(1:1/2:n2+1/2,1:1/2:n1+1/2); XI(:,end) = n2; YI(end,:) = n1;
...
%     h = compute_gaussian_filter([m m],sub*sigma2/(lambda*min(n1, n2)),[n1 n2]);
%     gx2 = perform_convolution(gx2,h);
%     gy2 = perform_convolution(gy2,h);
%     gxy = perform_convolution(gxy,h);
    gx2 = medfilt2(gx2, options.medfilt2_size);
    gy2 = medfilt2(gy2, options.medfilt2_size);
    gxy = medfilt2(gxy, options.medfilt2_size);
else
    % perform anisotropic hour-glass smoothing
    theta = linspace(0,pi,m_theta+1); theta(end) = [];
    for i=1:m_theta
        h = compute_hour_glass_filters( m*sub, sub*sigma2/(lambda*min(n1, n2)), sigmat, n1, theta(i) );
    ...
    theta = repmat( reshape(theta(:),[1 1 m_theta]), [sub*n1 sub*n2 1] );
    [tmp,I] = min( abs(Theta-theta),[],3 );
    I = reshape( (1:(sub*n2)*(sub*n1))' + (I(:)-1)*(sub*n2)*(sub*n1), sub*n1,sub*n2 );
    gx2 = Gx2(I);
    gy2 = Gy2(I);
    gxy = Gxy(I);
end
