function [x, fval, exitFlag, output] = bsSeisInv1DTraceByGlobalOptimization(d, G, xInit, Lb, Ub, regParam, parampkgs, options, regFunc)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This code is designed for 1D seismic inversion using regularization
% technique, and the otpimization is solved by global optimization
%
...
% -------------------------------------------------------------------------
    ...
    objFunc = @(x)bsSumFuncs(x, outputObjFcnPkgs);

    
    [x, fval, exitFlag, output] = bsHAGCSByShe2019(objFunc, Lb, Ub, options.GOOptions{:});
    output.regParam = regParam;
    output.parampkgs = inputObjFcnPkgs{2, 2};
end