...
costFunction = 'bsRosenbrock';           % F6 +/-5.12
npar  = 10;          % number of optimization variables
varLo = -5;         % Lower  band of parameter
varHi =  5;         % Higher band of parameter
%% Set COA parameters
numCuckooS = 20;             % number of initial population
minNumberOfEggs = 5;        % minimum number of eggs for each cuckoo
maxNumberOfEggs = 10;        % maximum number of eggs for each cuckoo
maxIter = 100;              % maximum iterations of the Cuckoo Algorithm
knnClusterNum = 3;          % number of clusters that we want to make
motionCoeff = 2;            % Lambda variable in COA paper, default=2
...
%             newParams = [newParams; params +  addingValue ];
            ...
            stepsize = 0.1 * step .* (params - goalPoint);
            
            addingValue = stepsize .* randn(size(params));
            newParams = [newParams; params +  addingValue ];
        end
...
%                                           motionCoeff * rand(1,npar) .*  (goalPoint  - tmpPositions(cntPosition,:));
            ...
            % In the next equation, the difference factor (nest-bestNest) means that 
            % when the solution is the bestNest solution, it remains unchanged.     
            stepsize = 0.01 * step .* (tmpPositions(cntPosition,:) - goalPoint);
...
%             nest = nest + stepsize .* randn(size(nest));
            ...
            tmpPositions(cntPosition,:) = tmpPositions(cntPosition,:) + stepsize .* randn(size(tmpPositions(cntPosition,:)));
