function [x,fval,exitFlag,output] = bsAHSACSByShe2019(objFunc, Lb, Ub, varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
% -------------------------------------------------------------------------
    ...
    for iter = 1 : params.maxIter
        ...
        [nests_with_ids, fitness] = bsSortNestsWithIdWithoutGradient(nests_with_ids, fitness, nNest);
        ...
        % calculate the fitness of the new population, the old individuals will not be replaced if their new
        % individuals are worse than the old ones
        [~, ~, nests, newFitness, ~] = bsGetBestNest(noGradientObjFunc, nests, newNest, fitness);
        ...
        % calculate the fitness of the new population, the old individuals will not be replaced if their new
        % individuals are worse than the old ones
        [bestFVal, bestNest, nests, newFitness, ~] = bsGetBestNest(noGradientObjFunc, nests, newNest, fitness);
...
function [nests, fitness] = bsSortNestsWithIdWithoutGradient(nests, fitness, nNewNest)
    ...
    piFi = [nests', fitness]; % the column looks like (nDim+1, 1)
    % Sort piFi with assending order of fitness (at the nDim*2+2-th column)
    piFiS = sortrows(piFi, nDim+2);

    % get the sorted nests with id information, and their corresponding fitness.
