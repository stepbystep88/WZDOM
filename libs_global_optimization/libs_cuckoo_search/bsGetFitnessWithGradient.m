function [nests, fitness, gradient] = bsGetFitnessWithGradient(fobj, nests, newNest, fitness, gradient)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% ----------------------------------------------------------------%
    ...
    for j = 1 : size(nests, 2)
        if newFitness(j) < fitness(j)
           % only replace the nests that have smaller fitness (smaller objective function value)
