function newNests = bsSwitchNestsWithBestNest(nests, gradient, bestNest, bestNestGradient, Lb, Ub, pa)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% ----------------------------------------------------------------%
    ...
    t = normrnd(0.5, 0.1, [1, nNest]);
    t = repmat(t, size(nests, 1), 1);
    
    stepsize = t .* (nests(:, randperm(nNest)) - nests(:, randperm(nNest)) + nests(:, randperm(nNest)) - nests(:, randperm(nNest)));
    newNests = bestNest + abs(stepsize) .* K;
